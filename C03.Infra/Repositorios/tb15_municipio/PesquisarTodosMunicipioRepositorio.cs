...
public class PesquisarTodosMunicipioRepositorio(CorretoraDbContext context) : IPesquisarTodosRepositorios<tb15_municipioModel>
{
    public async Task<(IEnumerable<tb15_municipioModel> dados, string mensagem, int codigo)> PesquisarTodosAsync(int pagina = 1, int quantidade = 20)
    {
        try
        {
            var entities = await context.Tabela15Municipio
                .Skip((pagina - 1) * quantidade)
                .Take(quantidade)
                .ToListAsync();
            return (entities, "Municípios listados com sucesso", 200);
        }
        catch (Exception ex)
        {
