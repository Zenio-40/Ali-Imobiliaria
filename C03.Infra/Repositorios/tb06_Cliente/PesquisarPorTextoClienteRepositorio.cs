...
public class PesquisarPorTextoClienteRepositorio(CorretoraDbContext context) : IPesquisarPorTextoRepositorio<tb06_clienteModel>
{
    public async Task<(IEnumerable<tb06_clienteModel> dados, string mensagem, int codigo)> PesquisarPorTextoAsync(string texto, int pagina = 1, int quantidade = 20)
    {
        try
        {
            var entities = await context.Tabela06Cliente.Where(c => c.Nome.Contains(texto))
                .Skip((pagina - 1) * quantidade)
                .Take(quantidade)
                .ToListAsync();
            return (entities, "Clientes encontrados", 200);
        }
        catch (Exception ex)
        {
